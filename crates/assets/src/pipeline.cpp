...
std::string find_esbuild() {
    if (command_exists("esbuild")) return "esbuild";
    if (command_exists("npx")) {
        auto ver = exec_cmd("npx esbuild --version 2>/dev/null");
        if (!ver.empty() && ver.find(".") != std::string::npos) return "npx esbuild";
    }
    if (file_exists("node_modules/.bin/esbuild")) return "./node_modules/.bin/esbuild";
    return "";
}

std::string find_lightningcss() {
    if (command_exists("lightningcss")) return "lightningcss";
    if (command_exists("npx")) {
        auto ver = exec_cmd("npx lightningcss --version 2>/dev/null");
        if (!ver.empty()) return "npx lightningcss";
    }
    if (file_exists("node_modules/.bin/lightningcss")) return "./node_modules/.bin/lightningcss";
    return "";
}

bool esbuild_available() { return !find_esbuild().empty(); }
bool lightningcss_available() { return !find_lightningcss().empty(); }

AssetResult process_css(const fs::path& input,
    ...
    if (!cli.empty()) {
        ...
        cmd += " -o " + output.string();
        if (opts.minify) cmd += " --minify";
        if (opts.sourcemap) cmd += " --source-map";
        if (!opts.targets.empty()) cmd += " --targets '" + opts.targets + "'";
        ...
        if (opts.minify) {
            ...
            // Trim.
            while (!css.empty() && css.front() == ' ') css.erase(0, 1);
            while (!css.empty() && css.back() == ' ') css.pop_back();
        }
...
AssetResult bundle_js(const std::vector<fs::path>& inputs,
                        const fs::path& output_dir,
    ...
    if (opts.minify) out_name += ".min";
    if (opts.format == "esm") out_name += ".mjs";
    else if (opts.format == "cjs") out_name += ".cjs";
    else out_name += ".js";
    auto output = output_dir / out_name;
    fs::create_directories(output_dir);
    ...
    if (opts.bundle) cmd += " --bundle";
    if (opts.minify) cmd += " --minify";
    if (opts.sourcemap) cmd += " --sourcemap";
    ...
    for ([[maybe_unused]] const auto& _pattern : cfg.css_patterns) {
        for (auto& entry : fs::directory_iterator(cfg.input_dir)) {
            ...
            if (entry.path().extension() != ".css" &&
                entry.path().extension() != ".scss") continue;
