...
// Resize an image.
[[nodiscard]] ImageResult resize(const fs::path& input,
                                    const fs::path& output,
                                    const ResizeOptions& opts);

// Convert image to WebP format.
[[nodiscard]] ImageResult to_webp(const fs::path& input,
                                     const fs::path& output,
                                     int quality = 80);

// Convert image to AVIF format.
[[nodiscard]] ImageResult to_avif(const fs::path& input,
                                     const fs::path& output,
                                     int quality = 80);
...
struct ImagePipelineConfig {
    ...
    std::vector<std::string> extensions = {".jpg", ".jpeg", ".png", ".gif", ".bmp", ".tiff"};
...
[[nodiscard]] std::vector<ImageResult> run_image_pipeline(const ImagePipelineConfig& cfg);

// Generate HTML <picture> element with multiple sources.
[[nodiscard]] std::string picture_tag(const fs::path& image_path,
