...
std::pair<int, int> get_dimensions(const fs::path& path) {
    ...
    if (command_exists("identify")) {
        auto out = exec_cmd(fmt::format("identify -format '%wx%h' '{}' 2>/dev/null", path.string()));
        // Parse "WIDTHxHEIGHT"
    ...
    if (command_exists("ffprobe")) {
        auto out = exec_cmd(fmt::format(
            "ffprobe -v error -select_streams v:0 "
            "-show_entries stream=width,height "
            "-of csv=p=0 '{}' 2>/dev/null", path.string()));
...
std::string find_tool(const std::string& tool_name) {
    if (command_exists(tool_name)) return tool_name;
    // Check common alternative locations.
    if (tool_name == "convert" && command_exists("magick")) return "magick";
    return "";
}

bool has_vipsthumbnail() { return command_exists("vipsthumbnail"); }
bool has_convert() { return command_exists("convert") || command_exists("magick"); }
bool has_cwebp() { return command_exists("cwebp"); }
bool has_avifenc() { return command_exists("avifenc"); }
...
ImageResult resize(const fs::path& input,
    ...
    if (has_vipsthumbnail()) {
        ...
        if (opts.width > 0 && opts.height > 0) {
            ...
            if (opts.maintain_aspect) size_str += ">";
        ...
        std::string cmd = fmt::format(
            "vipsthumbnail '{}' -s {} -o '{}' --Q={}",
            input.string(), size_str, out_path.string(), opts.quality);
...
ImageResult to_webp(const fs::path& input, const fs::path& output, int quality) {
    ...
    if (has_cwebp()) {
        std::string cmd = fmt::format(
            "cwebp -q {} '{}' -o '{}' 2>&1",
            quality, input.string(), out_path.string());
        exec_cmd(cmd);
    } else if (has_convert()) {
        std::string convert = find_tool("convert");
        std::string cmd = fmt::format(
            "{} '{}' -quality {} '{}'",
            convert, input.string(), quality, out_path.string());
        exec_cmd(cmd);
    } else {
...
ResponsiveResult generate_responsive(
    const fs::path& input,
    ...
    if (ext.empty()) ext = input.extension().string();
    ...
    if (!srcset_parts.empty()) {
        for (size_t i = 0; i < srcset_parts.size(); ++i) {
            if (i > 0) result.srcset_attribute += ", ";
...
std::vector<ImageResult> run_image_pipeline(const ImagePipelineConfig& cfg) {
    ...
    if (!fs::exists(cfg.input_dir)) return results;
    ...
    for (auto& entry : fs::recursive_directory_iterator(cfg.input_dir)) {
        ...
        // Resize.
        auto resize_result = resize(entry.path(),
                                      out_base.string() + entry.path().extension().string(),
                                      cfg.resize_opts);
        results.push_back(resize_result);

        // WebP conversion.
        if (cfg.generate_webp && has_cwebp()) {
            auto webp_result = to_webp(entry.path(),
                                         out_base.string() + ".webp",
                                         cfg.resize_opts.quality);
            results.push_back(webp_result);
        }

        // AVIF conversion.
        if (cfg.generate_avif && has_avifenc()) {
            auto avif_result = to_avif(entry.path(),
                                         out_base.string() + ".avif",
                                         cfg.resize_opts.quality);
        ...
        if (cfg.generate_responsive) {
            ...
            auto resp = generate_responsive(entry.path(), resp_dir,
                                             cfg.breakpoints, Format::WebP,
                                             cfg.resize_opts.quality);
...
std::string picture_tag(const fs::path& image_path,
    ...
    if (has_avifenc()) {
        ...
        for (size_t i = 0; i < breakpoints.size(); ++i) {
            if (i > 0) html += ", ";
            html += fmt::format("{}/{}-{}.avif {}w",
                                 dir.string(), stem, breakpoints[i].suffix, breakpoints[i].width);
        }
        html += "\">\n";
    }
    ...
    // Fallback img tag.
    html += fmt::format("  <img src=\"{}\" alt=\"{}\" loading=\"lazy\" decoding=\"async\">\n",
