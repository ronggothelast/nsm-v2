...
struct Parser {
  ...
  bool match(std::string_view tok) {
    skip_ws();
    if (pos + tok.size() > src.size()) return false;
    if (src.compare(pos, tok.size(), tok) != 0) return false;
    pos += tok.size();
    return true;
  }
  ...
  bool truthy(const std::string& s) {
    ...
    if (s == "0" || s == "false" || s == "nil") return false;
    return true;
  }

  std::string parse_atom() {
    skip_ws();
    if (pos >= src.size()) { ok = false; return ""; }
  ...
  std::string parse_unary() {
    ...
    if (pos < src.size() && src[pos] == '-' &&
        (pos + 1 >= src.size() || !std::isspace(static_cast<unsigned char>(src[pos + 1])))) {
      ...
      if (pos + 1 < src.size() &&
        ...
        auto v = parse_unary();
        double d = 0;
        if (to_double(v, d)) return number_to_str(-d);
        return v;
      }
    }
    return parse_atom();
  ...
  std::string parse_mul() {
    ...
    while (true) {
      ...
      if (match("*") || match("/") || match("%")) {
        ...
        double a = 0, b = 0;
        if (!to_double(lhs, a) || !to_double(rhs, b)) { ok = false; return ""; }
        if (op == '*') lhs = number_to_str(a * b);
        else if (op == '/') {
  ...
  std::string parse_cmp() {
    ...
    if (numeric) {
      ...
      if (op_str == "==") result = lhs == rhs;
      else if (op_str == "!=") result = lhs != rhs;
      else if (op_str == "<") result = lhs < rhs;
      else if (op_str == ">") result = lhs > rhs;
      else if (op_str == "<=") result = lhs <= rhs;
      else if (op_str == ">=") result = lhs >= rhs;
...
::nift::Expected<std::string, ::nift::Error> eval_expr(
    std::string_view expr, const ExprContext& ctx) {
...
::nift::Expected<bool, ::nift::Error> eval_bool(
  ...
  if (!r) return ::nift::unexpected<::nift::Error>(r.error());
  const auto& s = *r;
  if (s.empty() || s == "0" || s == "false" || s == "nil") return false;
  return true;
}
