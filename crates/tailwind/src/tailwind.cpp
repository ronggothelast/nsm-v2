...
fs::path generate_config(const TailwindConfig& cfg) {
    ...
    for (const auto& dir : cfg.content_dirs) {
        f << "    \"" << js_escape(dir) << "/**/*.{html,js,ts,jsx,tsx,vue,svelte,astro,md,php}\"\n";
    }
...
fs::path generate_input_css(const TailwindConfig& cfg,
...
TailwindResult compile(const TailwindConfig& cfg) {
    ...
    if (cli.empty()) {
        result.error_message = "Tailwind CLI not found. Install with: npm install -D tailwindcss";
        return result;
    }
...
TailwindResult watch(const TailwindConfig& cfg) {
    ...
    if (cfg.minify) cmd += " --minify";
    if (cfg.sourcemaps) cmd += " --source-map";
...
TailwindResult purge(const TailwindConfig& cfg,
