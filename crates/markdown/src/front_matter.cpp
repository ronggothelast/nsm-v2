...
nlohmann::json parse_yaml_lines(const std::vector<std::string>& lines, size_t start,
                                int base_indent) {
  ...
  while (i < lines.size()) {
    ...
    if (indent < base_indent)
      break;  // Dedented — return to caller.
    if (indent > base_indent) {
      ++i;
      continue;
    }  // Skip unexpected indent.
    ...
    if (val_str.empty() && i + 1 < lines.size()) {
      ...
      if (next_indent > base_indent) {
        ...
        while (i < lines.size()) {
          ...
          if (ni <= base_indent || lines[i].empty())
            break;
          ++i;
        }
        continue;
