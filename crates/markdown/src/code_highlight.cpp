...
Language detect_language(std::string_view info) {
    ...
    if (lower == "cpp" || lower == "c++" || lower == "cxx" || lower == "cc") return Language::Cpp;
    ...
    if (lower == "json")                                 return Language::Json;
    if (lower == "yaml" || lower == "yml")               return Language::Yaml;
    if (lower == "html" || lower == "htm")               return Language::Html;
    if (lower == "css")                                  return Language::Css;
    if (lower == "lua")                                  return Language::Lua;
...
std::string escape(std::string_view s) {
    ...
    for (char c : s) escape_char(c, out);
    return out;
}
...
bool is_cpp_keyword(std::string_view word) {
    static const char* keywords[] = {
        ...
        "uint32_t", "uint64_t", "size_t", "ptrdiff_t", "nullptr_t",
        "string", "string_view", "vector", "map", "set", "pair", "optional",
        "variant", "unique_ptr", "shared_ptr", "array", "span",
        "std", "fmt", "nift",
        nullptr
    };
...
bool is_js_keyword(std::string_view word) {
    static const char* keywords[] = {
        ...
        "throw", "true", "try", "typeof", "undefined", "var", "void", "volatile",
        "while", "with", "yield", "type", "namespace", "module", "declare",
        "readonly", "any", "never", "unknown", "string", "number", "boolean",
        nullptr
    };
    for (const char** kw = keywords; *kw; ++kw) {
        if (word == *kw) return true;
...
bool is_rust_keyword(std::string_view word) {
    static const char* keywords[] = {
        ...
        "Vec", "String", "Box", "Rc", "Arc", "Cell", "RefCell", "Mutex",
        nullptr
    };
    for (const char** kw = keywords; *kw; ++kw) {
        if (word == *kw) return true;
...
bool is_keyword(std::string_view word, Language lang) {
    switch (lang) {
        case Language::Cpp:
        case Language::C:         return is_cpp_keyword(word);
        case Language::Python:    return is_python_keyword(word);
        case Language::JavaScript:
        case Language::TypeScript: return is_js_keyword(word);
        case Language::Rust:      return is_rust_keyword(word);
        case Language::Go:        return is_go_keyword(word);
    ...
    while (i < len) {
        ...
        if (is_digit(c) || (c == '.' && i + 1 < len && is_digit(code[i + 1]))) {
            ...
            // Binary.
                ...
                i += 2;
                while (i < len && (code[i] == '0' || code[i] == '1')) ++i;
            }
            // Decimal/float.
            else {
    ...
    while (i < len) {
        ...
        if (c == '$') {
            ...
            if (i < len && code[i] == '{') {
                ...
                while (i < len && (is_alnum(code[i]) || code[i] == '?')) ++i;
            }
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_') {
            ...
            if (word == "if" || word == "then" || word == "else" || word == "elif" ||
                ...
                word == "export" || word == "source" || word == "exit" ||
                word == "set" || word == "unset" || word == "shift" ||
                word == "declare" || word == "readonly" || word == "trap" ||
                word == "true" || word == "false" || word == "echo" ||
                word == "cd" || word == "ls" || word == "cat" || word == "grep" ||
                word == "sed" || word == "awk" || word == "mkdir" || word == "rm" ||
                word == "cp" || word == "mv" || word == "chmod" || word == "chown" ||
                word == "sudo" || word == "apt" || word == "apt-get" || word == "pip" ||
                word == "npm" || word == "cargo" || word == "git" || word == "make" ||
                word == "cmake" || word == "curl" || word == "wget" ||
                word == "nift") {
                out += span("hl-keyword", word);
            } else {
                out += escape(word);
...
std::string highlight_html_code(std::string_view code) {
    ...
    while (i < len) {
        ...
        if (code.substr(i, 4) == "<!--") {
            ...
            if (end != std::string_view::npos) i = end + 3;
            else i = len;
            out += span("hl-comment", code.substr(start, i - start));
            continue;
        }
        ...
        if (c == '<') {
            ...
            if (is_closing) out += escape("/");
            out += span("hl-keyword", tag_name);

            // Attributes.
            while (i < len && code[i] != '>' && !(code[i] == '/' && i + 1 < len && code[i + 1] == '>')) {
                ...
                // Attribute name.
                size_t attr_start = i;
                while ((i < len && is_alnum(code[i])) || code[i] == '-' || code[i] == ':') ++i;
                if (i > attr_start) {
...
std::string highlight_css_code(std::string_view code) {
    ...
    while (i < len) {
        ...
        if (is_digit(c) || c == '.') {
            ...
            while (i < len && (is_digit(code[i]) || code[i] == '.')) ++i;
            // Units.
            while (i < len && (code[i] == 'p' || code[i] == 'x' || code[i] == 'e' ||
                   code[i] == 'm' || code[i] == 'r' || code[i] == 'v' ||
                   code[i] == 'w' || code[i] == 'h' || code[i] == '%' ||
                   code[i] == 'd' || code[i] == 'c' || code[i] == 'i' ||
                   code[i] == 'f' || code[i] == 's' || code[i] == 'l' ||
                   code[i] == 'b' || code[i] == 'k')) ++i;
            out += span("hl-number", code.substr(start, i - start));
            continue;
        }
    ...
    while (i < len) {
        ...
        if (c == '-' && i + 1 < len && code[i + 1] == '-') {
            ...
            if (i < len && code[i] == '[') {
                size_t j = i + 1;
                int level = 0;
                while (j < len && code[j] == '=') { ++level; ++j; }
                if (j < len && code[j] == '[') {
                    i = j + 1;
                    // Find matching ]=...]
                    std::string closer = "]";
                    for (int k = 0; k < level; ++k) closer += '=';
                    closer += ']';
                    size_t end = code.find(closer, i);
                    if (end != std::string_view::npos) i = end + closer.size();
                    else i = len;
                    out += span("hl-comment", code.substr(start, i - start));
                    continue;
                }
